...
#define SPEAKER_PIN PB12 // добавлен пин для динамика
...
int previousGrid[GRID_WIDTH][GRID_HEIGHT]; // предыдущее состояние игрового поля
...
// новые координаты области "next"
#define NEXT_PIECE_X (TFT_WIDTH - BORDER_WIDTH - 120) // same x as score
...
void setup() {
  ...
  pinMode(SPEAKER_PIN, OUTPUT); // инициализируем пин динамика как выход
...
void initGame() {
  ...
  memset(previousGrid, 0, sizeof(previousGrid)); // очистка предыдущего состояния поля
  score = 0;
  paused = false;
  ...
  // инициализируем начальные координаты фигуры
  previousPieceX = currentPieceX;
  previousPieceY = currentPieceY;
}
...
void spawnNewPiece() {
  ...
  // обновляем предыдущие координаты фигуры
  previousPieceX = currentPieceX;
  previousPieceY = currentPieceY;
}
...
void handleInput() {
  ...
  if (digitalRead(BUTTON_START) == LOW) { // пауза / возобновление игры
    ...
    // ждем, пока кнопка не будет отпущена, чтобы избежать дребезга
    while (digitalRead(BUTTON_START) == LOW);

    // не вызываем drawpausescreen() здесь.  он будет вызываться в loop().
    return;
  }
...
void movePiece(int dx, int dy) {
  if (canMove(dx, dy)) {
    // сначала стираем фигуру в старой позиции
    erasePiece(previousPieceX, previousPieceY, currentPieceType, currentPieceRotation);
...
// стирание фигуры в старой позиции
void erasePiece(int x, int y, int pieceType, int rotation) {
  for (int i = 0; i < 4; i++) {
    int eraseX = x + pieces[pieceType][rotation][i][0];
    int eraseY = y + pieces[pieceType][rotation][i][1];

    if (eraseX >= 0 && eraseX < GRID_WIDTH && eraseY >= 0 && eraseY < GRID_HEIGHT) {
      // Сначала закрашиваем блок черным цветом (внутреннюю часть)
      drawBlock(eraseX, eraseY, BLACK);
    }
  }
}
...
void fixPiece() {
  for (int i = 0; i < 4; i++) {
    ...
    if (x >= 0 && x < GRID_WIDTH && y >= 0 && y < GRID_HEIGHT) {
      grid[x][y] = currentPieceType + 1; // записываем тип фигуры + 1
    }
  }
...
// отрисовка игры
void drawGame() {
  // отрисовываем только изменившиеся блоки
  for (int x = 0; x < GRID_WIDTH; x++) {
    for (int y = 0; y < GRID_HEIGHT; y++) {
      if (grid[x][y] != previousGrid[x][y]) {
        // блок изменился, перерисовываем его
        if (grid[x][y] == 0) {
          // блок стал пустым, рисуем черным
          drawBlock(x, y, BLACK);
        } else {
          // блок занят, рисуем цветом фигуры
          drawBlock(x, y, getColor(grid[x][y] - 1));
        }
        // обновляем previousGrid
  ...
  // отрисовываем границу игрового поля
  drawBorders();
...
void drawNextPiece(int pieceType) {
  // выводим текст "next:" в нужном месте
  tft.setTextColor(WHITE);
  tft.setTextSize(2);
  tft.setCursor(NEXT_PIECE_X, NEXT_PIECE_Y - 20);  // отодвигаем текст немного вверх
  tft.println("Next:");
  ...
  for (int i = 0; i < 4; i++) {
    ...
    int pixelX = NEXT_PIECE_X + x * BLOCK_SIZE; // используем next_piece_x для выравнивания
...
void playGameOverSound() {
  ...
  for (int i = 0; i < sizeof(melody) / sizeof(melody[0]); i++) {
    tone(SPEAKER_PIN, melody[i], 500 / durations[i]); // применили формулу для длительности
    delay(500 / durations[i]);  // задержка между нотами
    noTone(SPEAKER_PIN);
    delay(10);
  }
}